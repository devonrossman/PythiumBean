...
Heatmap of 4 Cup Assay Variables, default
``{r}
library(pheatmap)
library(plyr)
Y <- (read.csv("/Users/devonrossman/Desktop/CSVforR/DryBean/AllCupApr19.csv"))
...
Y4<-( ddply(Y3, c("Var", "Title"), summarise,
            ...
            meanarea = mean(rtareatrial, na.rm=TRUE),
            meanemerge = mean(emergetrial, na.rm=TRUE)))
Y5<-as.data.frame(cbind(Y4$meanarea, Y4$meanlngth, Y4$meanwght, Y4$meanemerge))
#If you get the following error:
#Error in .Call.graphics(C_palette2, .Call(C_palette2, NULL)) : invalid graphics state
#Use the following code: dev.off() 
  rownames(Y5)<-Y4$Title
  colnames(Y5)<-names2<-c("Root Area", "Root Length", "Root Weight", "Emergence")

  #tableplot(Y5, ycol=names2)
g<-ggplot(Y4)
g+geom_point(aes(Y4$Title, Y4$meanwght, fill=Y4$Var, stat="identity"), position="dodge")+
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5))+
geom_point(aes(Y4$Title, Y4$meanarea, fill=Y4$Var, stat="identity"), position="dodge")
...
Y4<-( ddply(Y3, c("Var", "Title", "Temp"), summarise,
               N    = length(Sev),
...
ggplot(Y4, aes(x=reorder(Title, LL, median), y=meanSev)) + 
  geom_point(aes(fill=Y4$Temp), stat = "summary", fun.y="mean", size=2) + 
  theme_light() +
  theme(axis.text.x=element_text(angle=90, hjust = 1, vjust = 0.5, face="italic")) +
  scale_color_manual(values=c("#80b1d3","#fb8072"))+
  geom_errorbar(aes(limits, width=0.2, stat= "summary", fun.y="mean")) + 
