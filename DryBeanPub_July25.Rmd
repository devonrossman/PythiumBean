...
limits<-aes(ymax=mean+(SE*1.96), ymin=mean-(SE*1.96))
...
y <- ggplot(Yrtwght, aes(y=lsmean, 
                    x=Title))
y+geom_point(aes(x=reorder(Title, -lsmean), color=(lsmean+(SE*1.96)<0.4)), stat="identity", size=1.5)+
  geom_errorbar(limits, stat="identity", width=0.4)+
  theme_light()+
labs(title="Seedling Assay", x="Pythium spp.", y="Percent Relative to NIC")+
  facet_grid(Var~.)+
 theme(text=element_text(family="Arial Narrow"),
       plot.title=element_text(size=20, colour="black"),
axis.title=element_text(size=13, colour="black"), axis.text.x=element_text(size=8, color="black", angle=90, hjust=1, vjust=0.5), axis.text.y=element_text(size=10, color="black"), strip.text.y = element_text(size = 7, color="black"),
strip.text.x = element_text(size = 10, color="black"))+ scale_y_continuous(limits=c(-0.02,0.8), oob=rescale_none)
dev.off()
...
limits2<-aes(ymin=lsmean1$lsmean-(1.96*lsmean1$SE), ymax=lsmean1$lsmean+(1.96*lsmean1$SE))
jpeg("SevbyTemp.jpeg", width = 9.5, height = 5, units = 'in', res = 300)
g<-ggplot(lsmean1, aes(x=Title, y=lsmean, shape=Temp, color=lsmean-(SE*1.96)>0.08))
g+ geom_point(aes(x=reorder(Title, -lsmean), shape=Temp), position=position_dodge(width=0.5), size=2)+
  theme_light()+
  #facet_grid(Var~.)+
geom_errorbar(limits2, width=0.4, position=position_dodge(width=0.5))+
  labs(y="Disease Severity Index", x="")+
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5, color="black", size=10, face="italic"),
        axis.title.y=element_text(size=16), legend.text=element_text(angle=0, color="black", size=11), legend.text.align=0.5, legend.title=element_text(angle=0, size=11), legend.title.align=0.5, legend.position="right", axis.text.y=element_text(angle=90, hjust=0.5, vjust=1, color="black", size=12), legend.text.align=0.5) + scale_colour_manual(name = 'p<0.05', values = setNames(c('red','dark green'), c(T, F))) +
