...
Y4<-( ddply(Y3, c("Title", "Temp", "Clade"), summarise,
               N    = length(Sev),
            sev = mean(Sev),
...
limits<-aes(ymax=sev+(SE*1.96), ymin=sev-(SE*1.96))
Y4$Temp<-as.factor(Y4$Temp)

jpeg("IsoCladeSev.jpeg", width = 8, height = 5, units = 'in', res = 300)
y <- ggplot(Y4, aes(y=sev, 
                    #fill=sdtrt, 
                    x=factor(trt), shape=Temp, color=Clade))
y+geom_point(aes(x=reorder(trt, -sev)),  stat="identity", size=1.5)+
  theme_light()+
geom_errorbar(limits, stat="identity", width=0.4)+
labs(title="Seed Assay", x="Pythium spp.", y="Mean DSI Scores")+
 theme(plot.title=element_text(size=18, colour="black"),
axis.title=element_text(size=11, colour="black"), axis.text.x=element_text(size=7, color="black", angle=90, hjust=1, vjust=0.5), axis.text.y=element_text(size=10, color="black"), strip.text.y = element_text(size = 10),
...
y <- ggplot(Y4, aes(y=sev, 
                    x=Clade, shape=Temp, color=Clade))
...
scale_y_continuous(limits=c(0,1), oob=rescale_none)+
  scale_x_discrete(breaks=Y4$Clade)
  #geom_vline(xintercept=37.5)
dev.off()
```
...
Y3<-subset(Y, !(Y$rtareatrial=="NA") & !(Y$rtwghttrial=="NA"))
se <- function(x) sqrt(var(x, na.rm=TRUE)/(length(na.omit(x))))
descdist(Y3$rtwghttrial, discrete=FALSE)
...
YrtwghtRH <- Yrtwght[-c(32:62),]
YrtwghtZ <- Yrtwght[-c(1:31),]
limits<-aes(ymax=lsmean+(SE*1.96), ymin=lsmean-(SE*1.96))

jpeg("RtWghtRH.jpeg", width = 8, height = 3, units = 'in', res = 300)
yRH <- ggplot(YrtwghtRH, aes(y=lsmean, 
                    x=Title))
yRH+geom_point(aes(x=reorder(Title, -lsmean), color=(lsmean<0.55)), stat="identity", size=1.5)+
  geom_errorbar(limits, stat="identity", width=0.4)+
  theme_light()+
labs(title="Root Dry Weight", x="Pythium spp.", y="'Red Hawk'\n Root Dry Weight (g)")+
 theme(text=element_text(family="Arial Narrow"),
       plot.title=element_text(size=20, colour="black"),
...
strip.text.x = element_text(size = 9, color="black"))+ scale_y_continuous(limits=c(-0.02,0.8), oob=rescale_none)
dev.off()


jpeg("RtWghtZ.jpeg", width = 8, height = 3, units = 'in', res = 300)
yZ <- ggplot(YrtwghtZ, aes(y=lsmean, 
                    x=Title))
yZ+geom_point(aes(x=reorder(Title, -lsmean), color=(lsmean<0.3)), stat="identity", size=1.5)+
  geom_errorbar(limits, stat="identity", width=0.4)+
  theme_light()+
labs(title="Root Dry Weight", x="Pythium spp.", y="'Zorro' \nRoot Dry Weight (g)")+
 theme(text=element_text(family="Arial Narrow"),
       plot.title=element_text(size=20, colour="black"),
axis.title=element_text(size=9, colour="black"), axis.text.x=element_text(size=8, color="black", angle=90, hjust=1, vjust=0.5, face="italic"), axis.text.y=element_text(size=8, color="black"), strip.text.y = element_text(size = 7, color="black"),
strip.text.x = element_text(size = 9, color="black"))+ scale_y_continuous(limits=c(-0.02,0.8), oob=rescale_none)
...
jpeg("SevbyTemp.jpeg", width = 8, height = 5, units = 'in', res = 300)
g<-ggplot(lsmean1, aes(x=Title, y=lsmean, color=lsmean-(SE*1.96)>0.20), position=position_dodge(width=0.5))
g+ geom_point(aes(x=reorder(Title, -lsmean), shape=Temp), size=1)+
  theme_light()+
  #facet_grid(Var~.)+
geom_errorbar(limits, width=0.3, position=position_dodge(width=0.5))+
  labs(y="Disease Severity Index", x="")+
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5, color="black", size=9),
        axis.title.y=element_text(size=10), legend.text=element_text(angle=0, color="black", size=22), legend.text.align=0.5, legend.title=element_text(angle=0, size=24), legend.title.align=0.5, legend.position="right", axis.text.y=element_text(angle=90, hjust=0.5, vjust=1, color="black", size=9), legend.text.align=0.5) + scale_colour_manual(name = 'p<0.05', values = setNames(c('red','dark green'), c(T, F)))
dev.off()
```
...
#descdist(Y3$Sev, discrete=FALSE)
...
par(mfrow=c(2,4))
jpeg("AssayCompare.jpeg", width = 7, height = 5, units = 'in', res = 300)
